...
class RipenessPredictor(DetectionPredictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        p = ops.non_max_suppression_ripeness(preds[0],
                                    self.args.conf,
                                    self.args.iou,
        ...
        for i, pred in enumerate(p):
            ...
            if not len(pred):  # save empty boxes
                ...
                pred[:, :4] = ops.scale_boxes(img.shape[2:], pred[:, :4], orig_img.shape)
            results.append(Results(orig_img, path=img_path, names=self.model.names, boxes=pred[:, :6], masks=masks,ripeness=pred[:,-1]))
        return results
