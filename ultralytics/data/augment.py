...
class Mosaic(BaseMixTransform):
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        for labels in mosaic_labels:
            ...
            ripeness.append(labels['ripeness'])
        ...
        final_labels = {
            ...
            'ripeness': np.concatenate(ripeness, 0),
        ...
        final_labels['ripeness'] = final_labels['ripeness'][good]
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels['ripeness'] = np.concatenate([labels['ripeness'], labels2['ripeness']], 0)
        return labels
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1, labels2={}):
        ...
        for j in indexes[: round(self.p * n)]:
            ...
            ripeness = np.concatenate((ripeness, ripeness[[j]]), axis=0)
...
class Format:
    ...
    def __init__(
        ...
        self.return_ripeness = return_ripeness
    ...
    def __call__(self, labels):
        ...
        h, w = img.shape[:2]
        cls = labels.pop("cls")
        ripeness = labels.pop('ripeness')
        instances = labels.pop("instances")
        instances.convert_bbox(format=self.bbox_format)
        instances.denormalize(w, h)
        nl = len(instances)

        if self.return_mask or self.return_ripeness:
            if nl:
                # masks, instances, cls = self._format_segments(instances, cls, w, h)
                masks, instances, cls, ripeness = self._format_segments(instances, cls, ripeness, w, h)
        ...
        labels['ripeness'] = torch.from_numpy(ripeness) if nl else torch.zeros(nl)
    ...
    def _format_segments(self, instances, cls, ripeness, w, h):
        """
        ...
        if self.mask_overlap:
            ...
            cls = cls[sorted_idx]
            ripeness = ripeness[sorted_idx]
        else:
            masks = polygons2masks((h, w), segments, color=1, downsample_ratio=self.mask_ratio)

        return masks, instances, cls, ripeness
