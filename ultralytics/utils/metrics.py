...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        detections = detections[detections[:, 4] > self.conf] #大于置信度的
...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False,
                 on_plot=None, save_dir=Path(), names=(),eps=1e-16,prefix=''):
    """
    ...
    i = np.argsort(-conf) # 从大到小排序，返回对应索引
...
# def ap_per_class_ripe(tp, conf, pred_ripe, pred_cls, target_ripe, target_cls, plot=False,
#                  on_plot=None, save_dir=Path(), names=(),eps=1e-16,prefix=''):
#     """
#     Computes the average precision per class for object detection evaluation.
...
#     nc = unique_classes.shape[0]  # number of classes, number of detections, number of ripes
...
#     # Average precision, precision and recall curves
#     # ap, p_curve, r_curve = np.zeros((nc, tp.shape[1])), np.zeros((nc, 1000)), np.zeros((nc, 1000))
#     #初始化 对每个类别在每一个IOU阈值下，计算AP p r ripeness
#     ap, p_curve, r_curve = np.zeros((nc, tp.shape[1])), np.zeros((nc, 1000)), np.zeros((nc, 1000))

#     error = MAE(pred_ripe,target_ripe)


#     for ci, c in enumerate(unique_classes):
#         i = pred_cls == c #记录所有预测框是否是c类别 是c对应位置为True,否则为False
#         n_l = nt[ci]  # number of labels # gt框中c类别框数量
#         n_p = i.sum()  # number of predictions ## 预测框中c类别框数量
#         if n_p == 0 or n_l == 0: #如果没有预测到/ground truth没有标注 则略过类别c
#             continue
...
#     tp = (r * nt).round()  # true positives
#     fp = (tp / (p + eps) - tp).round()  # false positives
#     return tp, fp, p, r, f1, ap, unique_classes.astype(int), p_curve, r_curve, f1_curve, x, prec_values, error
...
class Metric(SimpleClass):
    ...
    def fitness(self):
        ...
        return (np.array(self.mean_results(),dtype=object) * w).sum()
...
class SegmentMetrics(SimpleClass):
    ...
    def process(self, tp_b, tp_m, conf, pred_cls, target_cls):
        """
        Processes the detection and segmentation metrics over the given set of predictions.处理给定预测结果检测和分割度量结果
...
class RipenessMetrics(SimpleClass):
    """
    Calculates and aggregates detection and segmentation metrics over a given set of classes.
