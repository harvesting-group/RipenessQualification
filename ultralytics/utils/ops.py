...
def non_max_suppression(
    ...
    # yolov8 is free anchor, so prediction only contains [xywh, cls] without conf
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            ...
            # v = torch.zeros((len(lb), nc + nm + 4 + 1), device=x.device)
            v[:, :4] = xywh2xyxy(lb[:, 1:5])  # box
            # v[:, :4] = xywh2xyxy(lb[:, 2:6])  # box
            v[range(len(lb)), lb[:, 0].long() + 4] = 1.0  # cls
            # v[range(len(lb)), lb[:, 0].long() + 4] = lb[:, 1]  # ripeness
        ...
        # boxes, scores, ripeness = x[:, :4] + c, x[:, 4] , x[:, 5]  # boxes (offset by class), scores        
        
        #print(boxes, scores, ripeness)
...
def non_max_suppression_ripeness(
        prediction,
        conf_thres=0.25,
        iou_thres=0.45,
        classes=None,
    ...
    bs = prediction.shape[0]  # batch size
    nc = nc or (prediction.shape[1] - 4 -1)  # number of classes - ripeness
    nm = prediction.shape[1] - nc - 4 -1
    mi = 4 + nc # mask start index
    xc = prediction[:, 4:mi].amax(1) > conf_thres  # candidates # [box, ripe, cls] ###
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            ...
            v = torch.zeros((len(lb), nc + nm + 4 + 1), device=x.device)
            v[:, :4] = xywh2xyxy(lb[:, 2:6])  # box
            v[range(len(lb)), lb[:, 0].long() + 4] = 1.0  # ripeness
            v[range(len(lb)), lb[:, 0].long() + 5] = lb[:, 1]  # cls
        ...
        # Detections matrix nx6 (xyxy, conf, cls)
        box, cls, mask, ripeness = x.split((4, nc, nm, 1), 1)

        if multi_label:
            i, j = torch.where(cls > conf_thres)
            # x = torch.cat((box[i], x[i, 4 + j, None], j[:, None].float(), mask[i]), 1)
            x = torch.cat((box[i], x[i, 4 + j, None], j[:, None].float(), mask[i], ripeness[i]), 1)
            # x = torch.cat((box[i], x[i, 4 + j, None], x[i, 4 + j +1, None], j[:, None].float(), mask[i], ripeness[i]), 1)
        else:  # best class only
            conf, j = cls.max(1, keepdim=True)
            # x = torch.cat((box, conf, ripeness, j.float(), mask), 1)[conf.view(-1) > conf_thres]
            x = torch.cat((box, conf, j.float(), mask, ripeness), 1)[conf.view(-1) > conf_thres]
